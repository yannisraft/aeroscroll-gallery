...
class Aeroscroll_Gallery_Admin
{
    ...
    public function aeroscroll_enqueue_scripts($hook_suffix)
    {
         ...
         * class.
         */
        // Enqueue the media uploader script (important for wp.media)
        wp_enqueue_media();
...
<?php
    ...
    // Update Version 1.0.12
    public function checkupdate_1012()
    {
        $debug = "checkupdate_1012";
        ...
        try {
            ...
            $table_aeroscroll = $wpdb->prefix . 'aeroscroll_gallery';
            $results = $wpdb->update(
                $table_aeroscroll,
                array(
                    'poweredbyactive' => 0,
                ),
                array('poweredbyactive' => 1)
            );
            ...
            if (count($table_cols) > 0) {
                ...
                if (!isset($table_row->autoscroll)) {
                    $update_query .= " ADD COLUMN media_gallery_id BIGINT UNSIGNED,";
                    $hasupdate = true;
                }
