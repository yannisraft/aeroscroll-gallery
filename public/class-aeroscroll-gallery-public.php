...
class Aeroscroll_Gallery_Public {
	 ...
	 * @since    1.0.0
	 */
	public function aeroscroll_enqueue_scripts() {
	 ...
	 * @since    1.0.0
	 */
	public function aeroscroll_register_grid_shortcode( $attrs, $params, $code ) {
		global $wpdb;
		...
		if ( count( $grid ) > 0 ) {
			...
			$has_scrollbar     = ( $grid[0]->scrollbar === 1 || $grid[0]->scrollbar === '1' ) ? 'true' : 'false';
			$cell_squared      = ( $grid[0]->cellsquared === 1 || $grid[0]->cellsquared === '1' ) ? 'true' : 'false';
			...
			if ( $grid[0]->scrollbar !== null ) {
				$hoveranimation = $grid[0]->hoveranimation;
			}

			$cellsize = 100;
			if ( isset( $grid[0]->cellsize ) ) {
				$cellsize = $grid[0]->cellsize;
				if ( gettype( $cellsize === 'string' ) ) {
					$cellsize = (int) $grid[0]->cellsize;
				}
			}
			...
			if ( isset( $grid[0]->sidegap ) ) {
				...
				if ( gettype( $sidegap === 'string' ) ) {
					$sidegap = (int) $grid[0]->sidegap;
				}
			}
			...
			if ( isset( $grid[0]->cellgap ) ) {
				...
				if ( gettype( $cellgap === 'string' ) ) {
					$cellgap = (int) $grid[0]->cellgap;
				}
			}

			$margin_x = 0;
			if ( isset( $grid[0]->marginX ) ) {
				$margin_x = $grid[0]->marginX;
				if ( gettype( $margin_x === 'string' ) ) {
					$margin_x = (int) $grid[0]->marginX;
				}
			}

			$margin_x = 0;
			if ( isset( $grid[0]->marginY ) ) {
				$margin_x = $grid[0]->marginY;
				if ( gettype( $margin_x === 'string' ) ) {
					$margin_x = (int) $grid[0]->marginY;
				}
			}
			$usemousewheel = 1;
			if ( isset( $grid[0]->usemousewheel ) ) {
				$usemousewheel = $grid[0]->usemousewheel;
				if ( gettype( $usemousewheel === 'string' ) ) {
					$usemousewheel = (int) $grid[0]->usemousewheel;
				}
			}
			$color_bg = '#ffffff00';
			...
			$aeroscroll_gallery_version   = $this->utils->aeroscroll_get_version();
            ...
            let AeroscrollGrid = await import('" . plugin_dir_url( __DIR__ ) . 'public/js/aeroscroll-gallery.js?v=' . $aeroscroll_gallery_iteration . "').then(module=>module?.default);            
            setTimeout(()=> {
